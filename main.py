...
try:
    ...
    import subprocess
    import threading
    def auto_install_shaper():
        try:
            subprocess.check_call([sys.executable, "-m", "pip", "install", "--quiet", "arabic-reshaper", "python-bidi"])
        except Exception:
            pass
    threading.Thread(target=auto_install_shaper, daemon=True).start()
...
class RecessedInput(Button):
    ...
    def __init__(self, label_text="", value_text="", bg_color=None, text_color=None, label_color_hex="#8c8c8c", **kwargs):
        ...
        self.halign = 'center'
        self.valign = 'middle'
        self.bind(pos=self.update_graphics, size=self.update_graphics, value_text=self.update_text, status_text=self.update_text, label_text=self.update_text, bg_color=self.update_graphics)
        self.update_text()
...
class MonitorView(FloatLayout):
    ...
    def redraw(self, *args):
        ...
        if app and app.soil_contaminated and not app.get_cut_segments():
            self.btn_help.text = get_rtl_text("راهنما") if get_lang() == "pe" else "Help"
            self.btn_help.font_name = "fonts/Vazirmatn-ExtraBold.ttf" if get_lang() == "pe" else "fonts/BRLNSDB.TTF"
...
class TwoPropScreen(BoxLayout):
    ...
    def twoPropCheckTimer_ftn(self, dt):
        ...
        if "memory" not in alarmState and alarm_twoProp == "dontSize":
            ...
            self.popup46 = Popup(title="", content=content, size_hint=(1, 0.4)) 
        ...
        if "memory" not in alarmState  and alarm_twoProp == "dontRef":
            ...
            content.add_widget(cap)
            self.popup45 = Popup(title="", content=content, size_hint=(0.8, 0.4)) 
        ...
        if for_twoProp_Mode == "ERROR:":
            ...
            self.popup44 = Popup(title="", content=content, size_hint=(0.8, 0.4)) 
    ...
    def waitLayout_twoProp(self):
        ...
        self.popup43 = Popup(title="", content=content, size_hint=(1, 1)) 
        ...
        if num == 0:
            if defaultRef != "":
                if "save" not in TwoScan_load_mode:
                    self.in_ref.label_text = "Soil Ref" + "  (" + "1" + ")"
    ...
    def load_from_json(self, filepath):
        ...
        try:
            ...
            content.add_widget(btn_close)
            popup49 = Popup(title="Size", content=content, size_hint=(0.8, 0.4)) 
...
class MainApp(App):
                    ...
                    if error == 0:
                        ...
                        try:
                            ...
                            if settingRead.get("pc_dir4") == "-":
                                R1 = R1 - valCalib if R1 >= valCalib else 0
                                R2 = R2 - valCalib if R2 >= valCalib else 0
                                R3 = R3 - valCalib if R3 >= valCalib else 0
                                R4 = R4 - valCalib if R4 >= valCalib else 0
                                R5 = R5 - valCalib if R5 >= valCalib else 0
                                R6 = R6 - valCalib if R6 >= valCalib else 0
                            else:
                                R1 += valCalib
                                R2 += valCalib
                                R3 += valCalib
                        ...
                        if "S1" in last:
                            get_4Prop_s1 = [result1, result2, result3, result4, result5, result6]
                            self.show_get_fourProp("S1,Good")
                        elif "S2" in last:
                            get_4Prop_s2 = [result1, result2, result3, result4, result5, result6]
                            self.show_get_fourProp("S2,Good")
                        elif "S3" in last: 
                            get_4Prop_s3 = [result1, result2, result3, result4, result5, result6]
                            self.show_get_fourProp("S3,Good")
                        elif "S4" in last:  
                            get_4Prop_s4 = [result1, result2, result3, result4, result5, result6]
                            self.show_get_fourProp("S4,Good")
                        elif "S5" in last: 
                            get_4Prop_s5 = [result1, result2, result3, result4, result5, result6]
                            self.show_get_fourProp("S5,Good")
                        elif "S6" in last:  
                            get_4Prop_s6 = [result1, result2, result3, result4, result5, result6]
                            self.show_get_fourProp("S6,Good") 
                        elif "S7" in last: 
                            get_4Prop_s7 = [result1, result2, result3, result4, result5, result6]
                            self.show_get_fourProp("S7,Good")
                        elif "S8" in last: 
                            get_4Prop_s8 = [result1, result2, result3, result4, result5, result6]
                            self.show_get_fourProp("S8,Good")
                    else:
                        for s_num in range(1, 9):
                            if f"S{s_num}" in last:
                                self.show_get_fourProp(f"S{s_num},Fail")
                except:
                    pass    
        except:
            pass
    ...
    def clear_vars(self):
        ...
        global twoPropValue
        global getRefCount 
        global getRefVa_1, getRefVa_2, getRefVa_3, getRefVa_4 
        global getRefVa_5, getRefVa_6, getRefVa_7, getRefVa_8 
        global get_4Prop_s1, get_4Prop_s2, get_4Prop_s3, get_4Prop_s4 
        global get_4Prop_s5, get_4Prop_s6, get_4Prop_s7, get_4Prop_s8
        ...
        twoPropValue = ""
        getRefCount = 0
        getRefVa_1 = getRefVa_2 = getRefVa_3 = getRefVa_4 = ""
        getRefVa_5 = getRefVa_6 = getRefVa_7 = getRefVa_8 = ""
        i = 0
        sizeType = ""  
        sizeTypeCounter = 0
        sizeTypeCount = 0
    ...
    def close_open_window(self):
        ...
        if showWiFi_error == 1:
            ...
            btn_close = Button(font_name=fontName,font_size=20,background_color=(1,0,0,1),text=bt, size_hint=(1, 0.2))
            content.add_widget(btn_close)
            self.popup21 = Popup(title="", content=content, size_hint=(0.8, 0.4)) 
    ...
    def show_get_fourProp(self,state):
        ...
        fail_label = get_rtl_text("خطا") if get_lang() == "pe" else "Fail"
        for s_idx, btn_s in enumerate([self.btn_size1, self.btn_size2, self.btn_size3, self.btn_size4, self.btn_size5, self.btn_size6, self.btn_size7, self.btn_size8], 1):
            if f"S{s_idx},Fail" in state:
                btn_s.background_color = (0.2, 0.2, 0.2, 0.8)
                btn_s.text = fail_label
            elif f"S{s_idx},Good" in state:
                btn_s.disabled = False

        if sizeTypeCounter == sizeTypeCount:
            self.clock_wait_animation.cancel()
            self.strl_wait.text = get_rtl_text("پایان") if get_lang() == "pe" else "Finished"
            self.btn_close_wait.disabled = False
            self.btnColor1.background_color=(0.8,0.8,0.8,0)
            self.btnColor2.background_color=(0.8,0.8,0.8,0)
            self.btnColor3.background_color=(0.8,0.8,0.8,0)
    ...
    def show_save_ref(self):
        try:  
            ...
            error = 0
            for r_val in [getRefVa_1, getRefVa_2, getRefVa_3, getRefVa_4, getRefVa_5, getRefVa_6, getRefVa_7, getRefVa_8]:
                if not r_val.isnumeric():
                    error = 1
                    break
            ...
            if error == 0:
                ...
                if get_lang() == "pe":
                    str_title = get_rtl_text("ذخیره شد")
                ...
                content.add_widget(btn_close)
                popup17 = Popup(title=str_title, content=content, size_hint=(0.8, 0.4)) 
                btn_close.bind(on_release=popup17.dismiss)
                popup17.open()
            else:
                fontName = "fonts/BRLNSDB.TTF" 
                str_title = "Error , Data is Corrupt"
                bt = "OK"

                if get_lang() == "pe":
                    str_title = get_rtl_text("خطا , دیتا معیوب است")
                    bt = get_rtl_text("تایید")
                    fontName = "fonts/Vazirmatn-ExtraBold.ttf"
                ...
                content.add_widget(btn_close)
                popup19 = Popup(title=str_title, content=content, size_hint=(0.8, 0.4)) 
            ...
            btn_close = Button(font_name=fontName,font_size=20,background_color=(0.9,0,0,1),text=bt, size_hint=(1, 0.2))
            content.add_widget(btn_close)
            popup18 = Popup(title=str_title, content=content, size_hint=(0.8, 0.4)) 
        ...
        try:
            ...
            if val.isnumeric() == True:
                ...
                if settingRead.get("pc_dir2") == "-":
                    tempStr = tempStr - valCalib if tempStr >= valCalib else 0
                else:
                ...
                if twoPropMode == "depth":
                    ...
                    if get_lang() == "pe":
                        pe_map = {
                            "natural": "طبیعی", "silver": "نقره", "gold": "طلا",
                            "copper": "مس", "brass": "برنج", "iron": "اهن",
                            "No Target": "بدون هدف", "water": "اب",
                            "Small Void": "حفره کوچک", "Medium Void": "حفره متوسط", "Big Void": "حفره بزرگ"
                        }
                        strVal = get_rtl_text(pe_map.get(model, model))

                    try:
                        if settingRead.get("CN") == "123":
                            num_map = {
                                "natural": "10", "silver": "20", "gold": "30", "copper": "40",
                                "brass": "50", "iron": "60", "No Target": "700", "water": "710",
                                "Small Void": "720", "Medium Void": "730", "Big Void": "740"
                            }
                            strVal = num_map.get(model, strVal)
                    except:
                        pass
                
                self.two_prop_lab2.text = strVal
            else:
                self.two_prop_lab2.text = get_rtl_text("خطا") if get_lang() == "pe" else "Error"
        except:
            self.two_prop_lab2.text = get_rtl_text("خطا") if get_lang() == "pe" else "Error"
    ...
    def start_pressed(self):
        ...
        content.add_widget(cap)
        self.popup16 = Popup(title="", content=content, size_hint=(0.8, 0.4)) 
    ...
    def update_status_label(self,state):
        if lang == "en":
            ...
            self.status.text = "WiFi : Connected" if state else "WiFi : DisConnected"
            self.status.color = (0, 0, 1, 1) if state else (1, 0, 0, 1)
        else:
            self.status.font_name = "fonts/Vazirmatn-ExtraBold.ttf"  
            self.status.text = get_rtl_text("وای فای : متصل") if state else get_rtl_text("وای فای : قطع ارتباط")
            self.status.color = (0, 0, 1, 1) if state else (1, 0, 0, 1)
    ...
    def update_text(self):
        LANG = {
            "en": {
                ...
                "fibo": "Fibo Scan",
                "gpr": "GPR Scan",
                "measure": "Measure 4 Scan",
                "list": "List",
                "menu": "2prop Scan Point Target",
                "reset": "Reset",
                "save": "Save",
                "recall": "Recall",
                "undo": "< Back Scan",
            ...
            "pe": {
                ...
                "fibo": "اسکن فیبو",
                "gpr": "اسکن GPR",
                "measure": "اندازه‌گیری ۴ اسکن",
                "list": "لیست",
                "menu": "اسکن ۲ پراپ نقطه‌ای",
                "reset": "پاکسازی",
                "save": "ذخیره",
                "recall": "فراخوانی",
                "undo": "< اسکن قبلی",
        ...
        is_pe = (lang == "pe")
        fn = "fonts/Vazirmatn-ExtraBold.ttf" if is_pe else "fonts/BRLNSDB.TTF"

        # Main Menu
        if hasattr(self, 'btn_memory'):
            self.btn_memory.text = get_rtl_text(LANG[lang]["memory"]) if is_pe else LANG[lang]["memory"]
            self.btn_memory.font_name = fn
        if hasattr(self, 'btn_depth'):
            self.btn_depth.text = get_rtl_text(LANG[lang]["depth"]) if is_pe else LANG[lang]["depth"]
            self.btn_depth.font_name = fn
        if hasattr(self, 'btn_scan'):
            self.btn_scan.text = get_rtl_text(LANG[lang]["scan"]) if is_pe else LANG[lang]["scan"]
            self.btn_scan.font_name = fn
        if hasattr(self, 'btn_setting'):
            self.btn_setting.text = get_rtl_text(LANG[lang]["setting"]) if is_pe else LANG[lang]["setting"]
            self.btn_setting.font_name = fn

        # 4-Prop Screen Controls
        if hasattr(self, 'btn_compare'):
            self.btn_compare.text = get_rtl_text(LANG[lang]["compare"]) if is_pe else LANG[lang]["compare"]
            self.btn_compare.font_name = fn
        if hasattr(self, 'btn_report'):
            self.btn_report.text = get_rtl_text(LANG[lang]["report"]) if is_pe else LANG[lang]["report"]
            self.btn_report.font_name = fn
        if hasattr(self, 'btn_start'):
            self.btn_start.text = get_rtl_text(LANG[lang]["start"]) if is_pe else LANG[lang]["start"]
            self.btn_start.font_name = fn

        if hasattr(self, 'in_w'):
            self.in_w.label_text = get_rtl_text(LANG[lang]["width"]) if is_pe else LANG[lang]["width"]
            self.in_w.font_name = fn
        if hasattr(self, 'in_l'):
            self.in_l.label_text = get_rtl_text(LANG[lang]["length"]) if is_pe else LANG[lang]["length"]
            self.in_l.font_name = fn
        if hasattr(self, 'in_ref'):
            self.in_ref.label_text = get_rtl_text(LANG[lang]["soil_ref"]) if is_pe else LANG[lang]["soil_ref"]
            self.in_ref.font_name = fn

        if hasattr(self, 'btn_geo'):
            self.btn_geo.text = get_rtl_text(LANG[lang]["geo"]) if is_pe else LANG[lang]["geo"]
            self.btn_geo.font_name = fn
        if hasattr(self, 'btn_fibo'):
            self.btn_fibo.text = get_rtl_text(LANG[lang]["fibo"]) if is_pe else LANG[lang]["fibo"]
            self.btn_fibo.font_name = fn
        if hasattr(self, 'btn_gpr'):
            self.btn_gpr.text = get_rtl_text(LANG[lang]["gpr"]) if is_pe else LANG[lang]["gpr"]
            self.btn_gpr.font_name = fn

        if hasattr(self, 'btn_measure'):
            self.btn_measure.text = get_rtl_text(LANG[lang]["measure"]) if is_pe else LANG[lang]["measure"]
            self.btn_measure.font_name = fn
        if hasattr(self, 'temp_list_fourScan'):
            self.temp_list_fourScan.text = get_rtl_text(LANG[lang]["list"]) if is_pe else LANG[lang]["list"]
            self.temp_list_fourScan.font_name = fn

        if hasattr(self, 'btn_menu'):
            self.btn_menu.text = get_rtl_text(LANG[lang]["menu"]) if is_pe else LANG[lang]["menu"]
            self.btn_menu.font_name = fn
        if hasattr(self, 'btn_reset'):
            self.btn_reset.text = get_rtl_text(LANG[lang]["reset"]) if is_pe else LANG[lang]["reset"]
            self.btn_reset.font_name = fn
        if hasattr(self, 'btn_save'):
            self.btn_save.text = get_rtl_text(LANG[lang]["save"]) if is_pe else LANG[lang]["save"]
            self.btn_save.font_name = fn
        if hasattr(self, 'btn_recall'):
            self.btn_recall.text = get_rtl_text(LANG[lang]["recall"]) if is_pe else LANG[lang]["recall"]
            self.btn_recall.font_name = fn

        if hasattr(self, 'btn_undo'):
            self.btn_undo.text = get_rtl_text(LANG[lang]["undo"]) if is_pe else LANG[lang]["undo"]
            self.btn_undo.font_name = fn
        if hasattr(self, 'btn_redo'):
            self.btn_redo.text = get_rtl_text(LANG[lang]["redo"]) if is_pe else LANG[lang]["redo"]
            self.btn_redo.font_name = fn
        if hasattr(self, 'exit4prop'):
            self.exit4prop.text = get_rtl_text(LANG[lang]["exit"]) if is_pe else LANG[lang]["exit"]
            self.exit4prop.font_name = fn
    ...
    def btn_close_wait_ftn(self,instance):
        ...
        for s_arr in [get_4Prop_s1, get_4Prop_s2, get_4Prop_s3, get_4Prop_s4, get_4Prop_s5, get_4Prop_s6, get_4Prop_s7, get_4Prop_s8]:
            if all(len(s_arr[x]) > 0 for x in range(6)):
                loadCount += 1
    ...
    def waitLayout(self):
        ...
        self.popup15 = Popup(title="", content=content, size_hint=(1, 1)) 
    ...
    def build_layout(self):
        ...
        i = 4
        str_pe = get_rtl_text("فا")
        self.btn_pe = Button(text=str_pe,font_name="fonts/Vazirmatn-ExtraBold.ttf",background_normal="",background_down="",font_size=26,background_color=colors[i],color=(0, 0, 0, 1))
    ...
    def updateSetting_ftn(self, *args):  
        ...
        try:
            ...
            if get_lang() == "pe":
                str_title = get_rtl_text("بروز رسانی شد")
                bt = get_rtl_text("تایید")
                fontName = "fonts/Vazirmatn-ExtraBold.ttf"
            ...
            btn_close = Button(font_name=fontName,font_size=20,background_color=(0,1,0,1),text=bt, size_hint=(1, 0.2))
            content.add_widget(btn_close)
            popup8 = Popup(title=str_title, content=content, size_hint=(0.8, 0.4)) 
            btn_close.bind(on_release=popup8.dismiss)
            popup8.open()
        except: 
            fontName = "fonts/BRLNSDB.TTF" 
            str_title = "update Error !"
            bt = "OK"

            if get_lang() == "pe":
                str_title = get_rtl_text("خطا در بروز رسانی !") 
                bt = get_rtl_text("تایید")
                fontName = "fonts/Vazirmatn-ExtraBold.ttf"
            ...
            btn_close = Button(font_name=fontName,font_size=20,background_color=(0.9,0,0,1),text=bt, size_hint=(1, 0.2))
            content.add_widget(btn_close)
            popup9 = Popup(title=str_title, content=content, size_hint=(0.8, 0.4)) 
    ...
    def old_scan_ftn(self , *args):
        global wcache, lcache
        global FourScan_load_mode, TwoScan_load_mode
        global get_4Prop_s1, get_4Prop_s2, get_4Prop_s3, get_4Prop_s4
        global get_4Prop_s5, get_4Prop_s6, get_4Prop_s7, get_4Prop_s8
        ...
        try:
            ...
            all_s = [get_4Prop_s1, get_4Prop_s2, get_4Prop_s3, get_4Prop_s4, get_4Prop_s5, get_4Prop_s6, get_4Prop_s7, get_4Prop_s8]
            for s_idx, s_arr in enumerate(all_s, 1):
                if all(len(s_arr[x]) > 0 for x in range(6)):
                    P1P2_val = s_arr[0]
                    P2P4_val = s_arr[1]
                    P4P3_val = s_arr[2]
                    ...
                    P1P4_val = s_arr[4]
                    P2P3_val = s_arr[5]
                    Soil_Ref_val = ref.get(f"refValue{s_idx}")
                    getFirst = s_idx
                    break
        except:
            pass
        ...
        if getFirst > 0:
            ...
            popup23 = Popup(title="", content=content, size_hint=(0.8, 0.4)) 
        ...
        self.update_text()
        for seg_b in self.segs:
            seg_b.trigger_action(0.1)
    ...
    def getRefValue(self, *args):
        global getRefCount, wifiSend, menuType
        global getRefVa_1, getRefVa_2, getRefVa_3, getRefVa_4 
        global getRefVa_5, getRefVa_6, getRefVa_7, getRefVa_8 
        ...
        str_title = "Please Wait"
        bt = "Approximate Time : 40 Seconds"

        if get_lang() == "pe":
            str_title = get_rtl_text("لطفا منتظر بمانید")
        ...
        self.popup10 = Popup(title=str_title, content=content, size_hint=(1, 1)) 
    ...
    def btn_yes_click(self, *args):
        ...
        btn_close = Button(font_name=fontName,font_size=20,background_color=(0.9,0,0,1),text=bt, size_hint=(1, 0.2))
        content.add_widget(btn_close)
        popup7 = Popup(title=str_title, content=content, size_hint=(0.8, 0.4)) 
    ...
    def btn_selectSize_ftn(self,instance):
        ...
        global get_4Prop_s1, get_4Prop_s2, get_4Prop_s3, get_4Prop_s4 
        global get_4Prop_s5, get_4Prop_s6, get_4Prop_s7, get_4Prop_s8 
        global menuType, sizeTypeCount, sizeType, sizeTypeCounter
        ...
        cbs = [self.checkbox1, self.checkbox2, self.checkbox3, self.checkbox4, self.checkbox5, self.checkbox6, self.checkbox7, self.checkbox8]
        for c_idx, cb in enumerate(cbs, 1):
            if cb.active:
                sizeType += f"{c_idx},"
                sizeTypeCount += 1
        ...
        if sizeTypeCount == 0:
            ...
            loadCount = 0
            for x in range(6):
                get_4Prop_s1[x] = get_4Prop_s2[x] = get_4Prop_s3[x] = get_4Prop_s4[x] = ""
                get_4Prop_s5[x] = get_4Prop_s6[x] = get_4Prop_s7[x] = get_4Prop_s8[x] = ""
            ...
            content.add_widget(labc)
            self.popup20 = Popup(title="", content=content, size_hint=(0.8, 0.4)) 
            Clock.schedule_once(lambda dt: self.popup20.dismiss(), 2.5)
            self.popup20.open()

    def toggleBtn_ftn(self,instance):
        for cb in [self.checkbox1, self.checkbox2, self.checkbox3, self.checkbox4, self.checkbox5, self.checkbox6, self.checkbox7, self.checkbox8]:
            cb.active = True
    ...
    def loadFourScan(self, *args):
        ...
        cbs = [self.checkbox1, self.checkbox2, self.checkbox3, self.checkbox4, self.checkbox5, self.checkbox6, self.checkbox7, self.checkbox8]
        for idx_cb, cb in enumerate(cbs, 1):
            sub_b = BoxLayout(orientation='vertical', spacing=0, padding=0)
            sub_b.add_widget(Label(text=str(idx_cb), font_size=20, font_name="fonts/BRLNSDB.TTF"))
            sub_b.add_widget(cb)
            contentG.add_widget(sub_b)
        ...
        self.popup14 = Popup(title="", content=content, size_hint=(1, 0.8)) 
    ...
    def twoPropsApp(self):
        global twoPropValue, menuType, twoPropMode
        menuType = "2prop"
        twoPropValue = ""
        ...
        exit_btn = Button(background_color=(1,0,0,1),font_size=20,font_name="fonts/BRLNSDB.TTF",text="Exit",size_hint=(None, None),pos_hint=({"center_x": 0.5, "center_y": 0.1}))
        ...
        if lang == "pe":
            ...
            exit_btn = Button(background_color=(1,0,0,1),font_size=20,font_name="fonts/Vazirmatn-ExtraBold.ttf",text=get_rtl_text("خروج"),size_hint=(None, None),pos_hint=({"center_x": 0.5, "center_y": 0.1}))

        exit_btn.bind(on_release=self.exit_ftn)
        self.gridy.add_widget(lab1)
        self.gridy.add_widget(self.two_prop_lab2)
        self.gridy.add_widget(exit_btn)
    ...
    def checkPassword(self , *args):
        ...
        if self.Pass.text == adminPass:
            ...
            btn_close = Button(font_name=fontName,font_size=20,background_color=(0.9,0,0,1),text=bt, size_hint=(1, 0.2))
            content.add_widget(btn_close)
            popup6 = Popup(title=str_title, content=content, size_hint=(0.8, 0.4)) 
    ...
    def setting_click(self, instance):
        ...
        if codeNeed == True:
            ...
            content.add_widget(btn_close)
            self.popup30 = Popup(title=str_title, content=content, size_hint=(0.8, 0.4)) 
    ...
    def temp_list_fourScan_ftn(self,*args):
        ...
        self.popup22 = Popup(title="", content=content, size_hint=(1, 1)) 
        ...
        if all(btn.disabled for btn in [self.size1_j, self.size2_j, self.size3_j, self.size4_j, self.size5_j, self.size6_j, self.size7_j, self.size8_j]):
            labc.text = get_rtl_text("لیست خالی است") if get_lang() == "pe" else "List Is Empty"
    ...
    def check_size_j(self):
        ...
        btns = [self.size1_j, self.size2_j, self.size3_j, self.size4_j, self.size5_j, self.size6_j, self.size7_j, self.size8_j]
        all_s = [get_4Prop_s1, get_4Prop_s2, get_4Prop_s3, get_4Prop_s4, get_4Prop_s5, get_4Prop_s6, get_4Prop_s7, get_4Prop_s8]

        for btn, s_arr in zip(btns, all_s):
            btn.disabled = not all(len(s_arr[x]) > 0 for x in range(6))
    ...
    def update_list_select(self ,*args ,num):
        ...
        try:
            ref = DataStore2("refDataValue.json", "SoilApp") 
            all_s = [get_4Prop_s1, get_4Prop_s2, get_4Prop_s3, get_4Prop_s4, get_4Prop_s5, get_4Prop_s6, get_4Prop_s7, get_4Prop_s8]
            target_s = all_s[num - 1]
            if all(len(target_s[x]) > 0 for x in range(6)):
                P1P2_val = target_s[0]
                P2P4_val = target_s[1]
                P4P3_val = target_s[2]
                P3P1_val = target_s[3]
                P1P4_val = target_s[4]
                P2P3_val = target_s[5]
                Soil_Ref_val = ref.get(f"refValue{num}")
        except:
            pass
    ...
    def switch_to_twoprop(self):
        global for_twoProp_Mode, sizeType, alarm_twoProp, twoProp_Mode, menuType
        global get_4Prop_s1, get_4Prop_s2, get_4Prop_s3, get_4Prop_s4
        global get_4Prop_s5, get_4Prop_s6, get_4Prop_s7, get_4Prop_s8
        global alarmState
    ...
    def switch_to_fourprop_yes(self,instance):
        ...
        global alarmState, for_twoProp_Mode, menuType
    ...
    def switch_to_fourprop(self):
        global for_twoProp_Mode, menuType, alarmState
    ...
    def translate(self, text):
        ...
        if get_lang() == "pe":
            pe_dict = {
                "gold": "طلا", "silver": "نقره", "copper": "مس", "Cooper": "مس",
                "brass": "برنج", "iron": "آهن", "natural": "طبیعی", "water": "آب",
                "Void": "حفره", "Small Void": "حفره کوچک", "Medium Void": "حفره متوسط",
                "Big Void": "حفره بزرگ", "No Target": "بدون هدف", "Unknown": "نامشخص",
                "Save Memory to:": "ذخیره حافظه در:", "Internal Storage": "حافظه داخلی",
                "Cancel": "انصراف", "Save Memory": "ذخیره حافظه", "Enter Filename": "نام فایل را وارد کنید",
                "Search: [Click to type]": "جستجو: [لمس کنید]", "No matches found": "موردی یافت نشد",
                "Select Scan File": "انتخاب فایل اسکن", "Save Image to:": "ذخیره تصویر در:",
                "Save Image": "ذخیره تصویر", "Recall Picture": "فراخوانی تصویر", "Close": "بستن",
                "Delete": "حذف", "Share": "اشتراک‌گذاری", "Work Report": "گزارش کار",
                "Target 1 Report": "گزارش هدف ۱", "Target 2 Report": "گزارش هدف ۲"
            }
            return get_rtl_text(pe_dict.get(text, text))
        if text == "copper":
            return "Cooper"
        return text
    ...
    def measure_4scan_click(self , *args):
        ...
        try:
            ...
            if ref.check("refDataValue.json", "SoilApp") == True:
                ref.save=({
                    "refValue1":"", "refValue2":"", "refValue3":"", "refValue4":"",
                    "refValue5":"", "refValue6":"", "refValue7":"", "refValue8":""
                })
    ...
    def exit4prop_click(self , *args):
        global FourScan_load_mode
        if FourScan_load_mode in ["save", "exit"]:
            self.layout.clear_widgets()   
            self.build_layout()
            self.btn_memory.trigger_action(0.1)
        elif FourScan_load_mode in ["load", "exit2"]:
            self.layout.clear_widgets()
            self.build_layout()

    def show_combined_save_popup(self, *args):
        content = BoxLayout(orientation='vertical', padding=10, spacing=10)
        btn_save_file = PlasticButton(text=get_rtl_text("ذخیره فایل اسکن") if get_lang() == "pe" else "Save file Scan", btn_color=[0.1, 0.5, 0.8, 1], size_hint_y=0.33)
        btn_save_pic = PlasticButton(text=get_rtl_text("ذخیره تصویر اسکن") if get_lang() == "pe" else "Save picture Scan", btn_color=[0.1, 0.6, 0.2, 1], size_hint_y=0.33)
        btn_cancel = PlasticButton(text=get_rtl_text("انصراف") if get_lang() == "pe" else "Cancel", btn_color=[0.8, 0.2, 0.2, 1], size_hint_y=0.33)
        
        if get_lang() == "pe":
            btn_save_file.font_name = "fonts/Vazirmatn-ExtraBold.ttf"
            btn_save_pic.font_name = "fonts/Vazirmatn-ExtraBold.ttf"
            btn_cancel.font_name = "fonts/Vazirmatn-ExtraBold.ttf"

        popup_title = get_rtl_text("گزینه‌های ذخیره") if get_lang() == "pe" else "Save Options"
        popup = Popup(title=popup_title, content=content, size_hint=(0.85, 0.45))
    ...
    def show_combined_recall_popup(self, *args):
        content = BoxLayout(orientation='vertical', padding=10, spacing=10)
        btn_recall_file = PlasticButton(text=get_rtl_text("فراخوانی فایل اسکن") if get_lang() == "pe" else "Recall file Scan", btn_color=[1, 0.4, 0.7, 1], size_hint_y=0.33)
        btn_recall_pic = PlasticButton(text=get_rtl_text("فراخوانی تصویر اسکن") if get_lang() == "pe" else "Recall picture Scan", btn_color=[0.5, 0.0, 0.5, 1], size_hint_y=0.33)
        btn_cancel = PlasticButton(text=get_rtl_text("انصراف") if get_lang() == "pe" else "Cancel", btn_color=[0.5, 0.5, 0.5, 1], size_hint_y=0.33)
        
        if get_lang() == "pe":
            btn_recall_file.font_name = "fonts/Vazirmatn-ExtraBold.ttf"
            btn_recall_pic.font_name = "fonts/Vazirmatn-ExtraBold.ttf"
            btn_cancel.font_name = "fonts/Vazirmatn-ExtraBold.ttf"

        popup_title = get_rtl_text("گزینه‌های فراخوانی") if get_lang() == "pe" else "Recall Options"
        popup = Popup(title=popup_title, content=content, size_hint=(0.85, 0.45))
    ...
    def update_target_boxes(self):
        ...
        self.t1.font_name = fn
        self.t2.font_name = fn

        if is_pe:
            self.t1.text = get_rtl_text("هدف ۱:\nX=                         Y=\nعمق (H) = —")
            self.t2.text = get_rtl_text("هدف ۲:\nX=                         Y=\nعمق (H) = —")
        else:
        ...
        for idx, target in enumerate(self.detected_circles):
            ...
            if is_pe:
                t_title = get_rtl_text(f"هدف {idx+1}:")
                t_val_lbl = get_rtl_text("مقدار:")
                t_val_str = f"{t_val_lbl} {val:.0f}"
                t_coord = f"X = {x_m:.2f} m        Y = {y_m:.2f} m"
                t_depth_lbl = get_rtl_text("عمق (H) =")
                t_depth_str = f"{t_depth_lbl} {depth:.2f} m"
                box.text = f"{t_title}  [b][size=11sp]{material}[/size][/b]      {t_val_str}\n{t_coord}\n{t_depth_str}"
            else:
    ...
    def save_memory_click(self, *args):
        ...
        if get_lang() == "pe":
            lbl.font_name = "fonts/Vazirmatn-ExtraBold.ttf"
            btn_internal.font_name = "fonts/Vazirmatn-ExtraBold.ttf"
            btn_cancel.font_name = "fonts/Vazirmatn-ExtraBold.ttf"
    ...
    def recall_memory_with_location(self, is_internal):
        ...
        bottom_row = BoxLayout(spacing=dp(3), size_hint_y=0.15)
        self.btn_delete_mode = PlasticButton(text=get_rtl_text("حالت حذف") if get_lang() == "pe" else "Delete Mode", btn_color=[0.8, 0.2, 0.2, 1])
        btn_cancel_select = PlasticButton(text=self.translate("Cancel"), btn_color=[0.5, 0.5, 0.5, 1])
        
        if get_lang() == "pe":
            self.btn_search_trigger.font_name = "fonts/Vazirmatn-ExtraBold.ttf"
            self.btn_delete_mode.font_name = "fonts/Vazirmatn-ExtraBold.ttf"
            btn_cancel_select.font_name = "fonts/Vazirmatn-ExtraBold.ttf"
        ...
        def populate_file_list(query=""):
            ...
            filtered_files = [f for f in self.recall_files_list if not query or query.lower() in f.lower()]
            if not filtered_files:
                lbl_no = Label(text=self.translate("No matches found"), size_hint_y=None, height=dp(40))
                if get_lang() == "pe":
                    lbl_no.font_name = "fonts/Vazirmatn-ExtraBold.ttf"
                self.file_list_grid.add_widget(lbl_no)
                return
            for f in filtered_files:
                display_name = f"🗑 {f}" if self.delete_mode else f
        ...
        def toggle_delete_mode(*args):
            self.delete_mode = not self.delete_mode
            if get_lang() == "pe":
                self.btn_delete_mode.text = get_rtl_text("خروج از حذف") if self.delete_mode else get_rtl_text("حالت حذف")
            else:
                self.btn_delete_mode.text = "Exit Delete" if self.delete_mode else "Delete Mode"
            self.btn_delete_mode.btn_color = [0.1, 0.6, 0.2, 1] if self.delete_mode else [0.8, 0.2, 0.2, 1]
            populate_file_list(self.current_search_query)
    ...
    def show_scan_options_popup(self, filename):
        ...
        try:
            ...
            if get_lang() == "pe":
                info_text += f"\n\n{get_rtl_text('عرض:')} {w_val}M | {get_rtl_text('طول:')} {l_val}M\n{get_rtl_text('مرجع خاک:')} {r_val}"
            else:
        ...
        btn_load = PlasticButton(text=get_rtl_text("بارگذاری") if get_lang() == "pe" else "Load", btn_color=[0.1, 0.6, 0.2, 1])
        ...
        btn_cancel = PlasticButton(text=self.translate("Cancel"), btn_color=[0.5, 0.5, 0.5, 1])
        
        if get_lang() == "pe":
            btn_load.font_name = "fonts/Vazirmatn-ExtraBold.ttf"
            btn_delete.font_name = "fonts/Vazirmatn-ExtraBold.ttf"
            btn_share.font_name = "fonts/Vazirmatn-ExtraBold.ttf"
            btn_cancel.font_name = "fonts/Vazirmatn-ExtraBold.ttf"
        ...
        options_popup = Popup(title=self.translate("Save Memory"), content=content, size_hint=(0.85, 0.5), auto_dismiss=False)
    ...
    def recall_jpeg_with_location(self, is_internal):
        ...
        bottom_row = BoxLayout(spacing=dp(3), size_hint_y=0.15)
        self.btn_pic_delet_toggle = PlasticButton(text=get_rtl_text("حالت حذف") if get_lang() == "pe" else "Delete Mode", btn_color=[0.8, 0.2, 0.2, 1])
        btn_cancel_select = PlasticButton(text=self.translate("Cancel"), btn_color=[0.5, 0.5, 0.5, 1])
        
        if get_lang() == "pe":
            self.btn_pic_search_trigger.font_name = "fonts/Vazirmatn-ExtraBold.ttf"
            self.btn_pic_delet_toggle.font_name = "fonts/Vazirmatn-ExtraBold.ttf"
            btn_cancel_select.font_name = "fonts/Vazirmatn-ExtraBold.ttf"
        ...
        def populate_pic_file_list(query=""):
            ...
            if not filtered_files:
                ...
                if get_lang() == "pe":
                    lbl_no.font_name = "fonts/Vazirmatn-ExtraBold.ttf"
                self.pic_file_list_grid.add_widget(lbl_no)
                return
            for f in filtered_files:
                display_name = f"🗑 {f}" if self.pic_delete_mode else f
                btn_clr = [0.6, 0.1, 0.1, 1] if self.pic_delete_mode else [0.3, 0.3, 0.3, 1]
        ...
        def toggle_pic_delete_mode(*args):
            self.pic_delete_mode = not self.pic_delete_mode
            if get_lang() == "pe":
                self.btn_pic_delet_toggle.text = get_rtl_text("خروج از حذف") if self.pic_delete_mode else get_rtl_text("حالت حذف")
            else:
                self.btn_pic_delet_toggle.text = "Exit Delete" if self.pic_delete_mode else "Delete Mode"
            self.btn_pic_delet_toggle.btn_color = [0.1, 0.6, 0.2, 1] if self.pic_delete_mode else [0.8, 0.2, 0.2, 1]
            populate_pic_file_list(self.current_pic_search_query)
    ...
    def show_image_viewer(self, filepath):
        ...
        if get_lang() == "pe":
            btn_close.font_name = "fonts/Vazirmatn-ExtraBold.ttf"
            btn_delete.font_name = "fonts/Vazirmatn-ExtraBold.ttf"
            btn_share.font_name = "fonts/Vazirmatn-ExtraBold.ttf"
    ...
    def show_popup(self, title, text):
        ...
        if get_lang() == "pe":
            btn.font_name = "fonts/Vazirmatn-ExtraBold.ttf"
    ...
    def undo_click(self, *args):
        if self.history_index > 0:
            ...
            msg = get_rtl_text("مرحله قبلی دیگری وجود ندارد.") if get_lang() == "pe" else "No more undo steps available."
            self.show_popup("Info", msg)
    ...
    def redo_click(self, *args):
        if self.history_index < len(self.history) - 1:
            ...
            msg = get_rtl_text("مرحله بعدی دیگری وجود ندارد.") if get_lang() == "pe" else "No more redo steps available."
            self.show_popup("Info", msg)

    def show_contamination_help(self, *args):
        if get_lang() == "pe":
            msg = get_rtl_text("محیط اسکن شده دارای آلودگی بسیار شدید است. این آلودگی یا به فلزات سطحی مربوط است و یا به ویژگی‌های خاک این منطقه. جهت اطمینان، ابتدا فلزات سطحی را با یک فلزیاب لوپ از زمین خارج نموده و مجدداً اسکن نمایید.")
            title = get_rtl_text("هشدار آلودگی")
        else:
            msg = "The scanned environment has very severe contamination, this contamination is either related to surface metals or to the soil properties of this spot. To be sure, please first remove the surface metals from the ground with a loop metal detector and scan again, if the problem is not resolved, this environment is dirty."
    ...
    def show_report_popup(self, *args):
        ...
        btn_close_text = get_rtl_text("بستن") if get_lang() == "pe" else "Close"
        btn_next_text = get_rtl_text("هدف بعدی") if get_lang() == "pe" else "Next Target"

        self.btn_close_report = PlasticButton(text=btn_close_text, btn_color=[0.8, 0.2, 0.2, 1], size_hint_x=0.5)
        self.btn_next_report = PlasticButton(text=btn_next_text, btn_color=[0.11, 0.51, 0.84, 1], size_hint_x=0.5)
        
        if get_lang() == "pe":
            self.btn_close_report.font_name = "fonts/Vazirmatn-ExtraBold.ttf"
            self.btn_next_report.font_name = "fonts/Vazirmatn-ExtraBold.ttf"
    ...
    def toggle_report_target(self, *args):
        ...
        if get_lang() == "pe":
            self.btn_next_report.text = get_rtl_text("هدف بعدی") if self.report_target_idx == 0 else get_rtl_text("هدف قبلی")
        else:
            self.btn_next_report.text = "Next Target" if self.report_target_idx == 0 else "Prev Target"
        self.build_report_layout()
    ...
    def build_report_layout(self):
        ...
        if is_pe:
            if ref < 200:
                soil_type = get_rtl_text("خیلی مرطوب / گل‌آلود")
            elif ref < 600:
                soil_type = get_rtl_text("مرطوب / نرمال")
            elif ref < 1500:
                soil_type = get_rtl_text("نیمه‌خشک")
            else:
                soil_type = get_rtl_text("خیلی خشک / شنی")
        else:
            if ref < 200:
        ...
        if is_pe:
            rows = [
                (get_rtl_text("نام هدف:"), get_rtl_text(f"هدف {target_num}")),
                (get_rtl_text("جنس هدف:"), material_trans),
                (get_rtl_text("مختصات X:"), f"{x_m:.2f} m"),
                (get_rtl_text("مختصات Y:"), f"{y_m:.2f} m"),
                (get_rtl_text("عمق هدف:"), f"{depth_m:.2f} m"),
                (get_rtl_text("قطر آلودگی:"), f"{diameter_cm:.1f} cm"),
                (get_rtl_text("میزان خطا:"), f"±{error_cm} m"),
                (get_rtl_text("فرکانس ارسال (TX):"), pointer_y_freq),
                (get_rtl_text("فرکانس دریافت (RX):"), pointer_y_freq_rx),
                (get_rtl_text("محدوده فرکانس:"), txt_range),
                (get_rtl_text("مقاومت مرجع خاک:"), f"{ref:.0f} Ohm"),
                (get_rtl_text("تخمین نوع خاک:"), soil_type),
                (get_rtl_text("کنتراست سیگنال:"), contrast_str)
            ]
            title_text = get_rtl_text("گزارش هدف ۱") if target_num == 1 else get_rtl_text("گزارش هدف ۲")
        else:
            rows = [
            ...
            title_text = f"[b]{'Target 1 Report' if target_num == 1 else 'Target 2 Report'}[/b]"
        ...
        for k, v in rows:
            k_lbl = Label(
                ...
                size_hint_y=None, height=dp(25), halign='left' if not is_pe else 'right', valign='middle', padding=(dp(15), 0),
                font_name=fn
            )
            ...
            v_lbl = Label(
                ...
                size_hint_y=None, height=dp(25), halign='right' if not is_pe else 'left', valign='middle', padding=(dp(15), 0),
                font_name=fn
            )
