...
class TwoPropScreen(BoxLayout):
    ...
    def load_first_ref_in_two(self, num):
        ...
        try:
            ...
            v1 = ref.get("refValue1", "")
            defaultRef = str(v1) if v1 and str(v1) != "None" else ""

            if num == 1:
                v = ref.get("refValue1", "")
                tempValue = str(v) if v and str(v) != "None" else ""
            elif num == 2:
                v = ref.get("refValue2", "")
                tempValue = str(v) if v and str(v) != "None" else ""
            elif num == 3:
        ...
        if num == 0:
            ...
            if tempValue != "":
                self.in_ref.label_text = f"Soil Ref  ({num})"
        ...
        try:
            ...
            val_res = settingRead.get("res", "220 R")
            if val_res and str(val_res) != "None":
                res = str(val_res)
        except Exception:
            pass
        ...
        try:
            if self.in_ref.value_text and len(self.in_ref.value_text) > 0:
...
class MainApp(App):
    ...
    def update_status_label(self,state):
        ...
        if lang == "en":
            ...
            if state == True:
                ...
                self.status.color = color=(0, 0, 1, 1)
            else:    
                self.status.text = "WiFi : DisConnected"
                self.status.color = color=(1, 0, 0, 1)
        else:
    ...
    def update_text(self):
        global lang
        if not lang or lang not in ["en", "pe"]:
            lang = "en"
        ...
        if lang == "en":
            self.btn_memory.text = LANG["en"]["memory"]
            self.btn_depth.text = LANG["en"]["depth"]
            self.btn_scan.text = LANG["en"]["scan"]
            self.btn_setting.text = LANG["en"]["setting"]  
            ...
            self.btn_memory.text = get_rtl_text(LANG["pe"]["memory"])
            self.btn_depth.text = get_rtl_text(LANG["pe"]["depth"])
            self.btn_scan.text = get_rtl_text(LANG["pe"]["scan"])
            self.btn_setting.text = get_rtl_text(LANG["pe"]["setting"])
    ...
    def build(self):
        ...
        try:
            langu = DataStore2("languageDataVal.json", "SoilApp")
            saved_lang = langu.get("language", "en")
            if saved_lang in ["en", "pe"]:
                lang = saved_lang
            else:
                lang = "en"
                langu.set("language", "en")
    ...
    def switch_to_twoprop(self):
        ...
        try:
            ref = DataStore2("refDataValue.json", "SoilApp")
            val_ref = ref.get("refValue1", "")
            if val_ref and str(val_ref) != "None":
                refValue = str(val_ref)
        except Exception:
            pass
        ...
        if not refValue or refValue == "":
            alarm_twoProp = "dontRef"
        elif not sizeType or sizeType == "":
