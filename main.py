...
class TwoPropScreen(BoxLayout):
    ...
    def load_first_ref_in_two(self, num):
        ...
        try:
            ...
            val_d = ref.get("refValue1")
            if val_d:
                defaultRef = str(val_d)
            if num >= 1 and num <= 8:
                val_num = ref.get(f"refValue{num}")
                if val_num:
                    tempValue = str(val_num)
        except Exception:
            pass
    ...
    def execute_save_memory(self, is_internal, filename):
        ...
        state = {
            ...
            "ref_soil": str(self.ref_soil_val) if hasattr(self, 'ref_soil_val') else self.in_ref.value_text,
        ...
        try:
            folder = self.app.get_writable_folder(os.path.join("Cornix Winner", "2 prop Scan", "File Scan"))
    ...
    def execute_save_picture(self, is_internal, filename):
        ...
        try:
            ...
            # ذخیره تصویر با متد PIL جهت اطمینان از خروجی سالم در اندروید
            ...
            # اطلاع‌رسانی به گالری و مدیریت فایل اندروید
            try:
...
class MainApp(App):
    ...
    def get_writable_folder(self, folder_subpath):
        try:
            ...
            if platform == 'android':
                ...
                doc_dir = Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOCUMENTS).getAbsolutePath()
                folder = os.path.join(doc_dir, folder_subpath)
    ...
    def switch_to_twoprop(self):
        ...
        if not refValue or refValue == "None":
    ...
    def get_storage_root(self, is_internal=True):
        try:
            ...
            if platform == 'android':
                ...
                if doc_dir:
                    return doc_dir.getAbsolutePath()
